...
function achievementsReducer(achievements, actions) {
	const { habits, mainDiary, isInitialRender } = actions;

	// If this is the initial render and no achievements are unlocked, return early
	if (isInitialRender && !achievements.some(a => a.isUnlocked)) {
		return achievements;
	}
	...
	const updatedAchievements = achievements.map(
		(a) => {
			...
			if (shouldUnlock) {
				handleUnlockAchievement(updatedA);
				hasChanges = true;
			}
	...
	// Only save to localStorage if there were actual changes
	if (hasChanges) {
		const basicAchievementsInfo = updatedAchievements
